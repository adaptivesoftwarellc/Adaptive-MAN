...
- [x] All admin endpoints write audit rows — `POST /api/admin/apps` and key mint emit rows; key-revoke now emits `admin.key.revoked` (10.6, Phase 4). `AddAdminAuth` gates the whole `/api/admin` group.
- [x] Read-only audit view — **backend** (PR C, branch `phase-8/7-audit-logging-backend`): paginated, filterable `GET /api/admin/audit` (filters: `action`, `app` slug/id, `from`/`to`, `page`/`page_size`; `OccurredAt DESC`; admin-gated; `{ total, page, page_size, rows }` envelope). **UI shipped in 10.6** (Phase 4, branch `phase-10/6-admin-ui`) — the admin audit-log page consumes this endpoint. See [`docs/work/pr-c-investigation.md`](docs/work/pr-c-investigation.md).
...
**Acceptance criteria:**
- [x] Dashboard navigation adds "Admin" links (Apps + Audit log), gated on the 8.6 Admin role (`RequireAdmin`); Keys reached per-environment from the Apps page.
- [x] Apps page: list + create (`POST /api/admin/apps`), view environments + active/total key counts (new admin-scoped `GET /api/admin/apps`).
- [x] Keys page (per env): mint (plaintext shown once with copy button + not-retrievable warning), revoke, list with masked key id + created + last-used (`GET .../keys`; `ApiKey.LastUsedAt` added + stamped on resolve, throttled to ≤1 write/key/5min).
- [x] Audit log page: read-only, consumes `GET /api/admin/audit` with an action filter + pagination (closes 8.7's read-only audit view).
- [x] New endpoints: `POST /api/admin/apps/{slug}/environments/{env}/keys/{id}/revoke` (writes `admin.key.revoked`); `GET .../keys`; admin `GET /api/admin/apps`. (`GET /api/admin/audit` shipped in PR C.)
- [x] Smoke test: mint key → use on `/api/ingest/events` (202) → revoke → confirm 401. Implemented as a backend integration test (`AdminEndpointsTests.RevokeKey_MintUseRevoke_ThenIngestReturns401_AndAudits`) — the repo has no Cypress/Playwright harness, and the backend flow is the reliable equivalent.

**Note:** this overlaps with 8.7's "read-only audit view" — the audit log page in 10.6 IS the 8.7 view. ✅ Closed with 10.6 (Phase 4, branch `phase-10/6-admin-ui`); the revoke endpoint emits the `admin.key.revoked` row 8.7 anticipated.
