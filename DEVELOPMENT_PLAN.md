...
- [ ] `obs-api-dev` provisioned with the admin key + a test app/key for the 4.11 harness — closes the deferred line in 5.7's acceptance criteria. **(Brandon: set the `ObservabilityAdminKey` KV secret, then run the two `POST /api/admin/...` calls. Endpoint surface is live as of this branch.)**
